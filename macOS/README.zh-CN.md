...
## Preview 3：圆角图标、窄窗口与源首页
- 窗口最小内容尺寸为 320 × 420 点；低于 680 点宽度时切换为顶部导航，阅读底部工具栏可换行。
- 构建时用 AppKit 对官方图标添加圆角与透明边距，再生成全部 ICNS 尺寸。
- 浏览页自动载入源提供的首页，支持首页漫画分区、分类入口、首页筛选入口与分页。没有首页的源回退到分类或搜索。
- 点击右上角列表/网格按钮切换海报墙，网格列数随宽度调整。点击房子返回源首页，分类按钮可选择源提供的列表。
- 首页布局目前统一呈现为网格或列表；轮播自动播放、排行榜编号、章节摘要和全套搜索筛选编辑器尚未移植。封面已接入源的图片请求头与封面处理接口，并加入内存缓存；需要网页登录的站点仍可能无法显示封面。
